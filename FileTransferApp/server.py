...
link = "http://" + s.getsockname()[0] + ":" + str(PORT) # the directory to access local files  http://ipaddr:8081 
# client socket address : s.getsockname()[0]

url = pyqrcode.create(link)  
url.svg("qr-dfiles.svg", scale=8) # save the QR code as svg file
webbrowser.open('D:\\SSN\\SEM5\\NW\\FileTransferApp\\renderDfile.html') # render the html file (containing the QR code and the link to access the files) in a webbrowser
